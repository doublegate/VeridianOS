...
- Unsafe policy: every unsafe block needs `// SAFETY:` explaining invariants and preconditions; prefer the `GlobalState` pattern over `static mut` (only 7 justified remain). See `docs/unsafe-policy.md` for the binding rules (must-read before touching `unsafe`).  
