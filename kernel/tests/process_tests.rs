...
use veridian_kernel::{
    ...
    process::{
        self, Process, ProcessId, ProcessState, Thread, ThreadId, ThreadState,
        sync::{KernelMutex, KernelSemaphore, KernelCondVar, KernelRwLock, KernelBarrier},
    },
    serial_println,
    test_framework::{BenchmarkRunner, cycles_to_ns, read_timestamp},
};
...
fn test_process_creation() {
    ...
    kernel_assert_eq!(process.state.load(Ordering::Relaxed), ProcessState::Created as u8);
...
fn test_process_state_transitions() {
    ...
    kernel_assert_eq!(process.state.load(Ordering::Relaxed), ProcessState::Ready as u8);
    
    // Ready -> Running
    process.set_state(ProcessState::Running);
    kernel_assert_eq!(process.state.load(Ordering::Relaxed), ProcessState::Running as u8);
    
    // Running -> Blocked
    process.set_state(ProcessState::Blocked);
    kernel_assert_eq!(process.state.load(Ordering::Relaxed), ProcessState::Blocked as u8);
    ...
    kernel_assert_eq!(process.state.load(Ordering::Relaxed), ProcessState::Exited as u8);
...
fn test_thread_creation() {
    ...
    kernel_assert_eq!(thread.state.load(Ordering::Relaxed), ThreadState::Creating as u8);
}
...
fn test_thread_state_transitions() {
    ...
    kernel_assert_eq!(thread.state.load(Ordering::Relaxed), ThreadState::Ready as u8);
    
    // Ready -> Running
    thread.set_state(ThreadState::Running);
    kernel_assert_eq!(thread.state.load(Ordering::Relaxed), ThreadState::Running as u8);
    
    // Running -> Blocked
    thread.set_state(ThreadState::Blocked);
    kernel_assert_eq!(thread.state.load(Ordering::Relaxed), ThreadState::Blocked as u8);
    ...
    kernel_assert_eq!(thread.state.load(Ordering::Relaxed), ThreadState::Exited as u8);
}
...
fn test_atomic_operations() {
    ...
    kernel_assert!(COUNTER.compare_exchange(
        false, 
        true, 
        Ordering::SeqCst, 
        Ordering::SeqCst
