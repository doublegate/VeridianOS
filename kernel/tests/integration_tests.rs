...
fn test_process_with_ipc_and_capabilities() {
    ...
    // Create IPC endpoint
    let (endpoint_id, endpoint_cap) =
        create_endpoint(ProcessId(100)).expect("Failed to create endpoint");
...
fn test_capability_revocation() {
    ...
    // Verify it exists
    assert!(process.capability_space.lock().lookup(cap_token).is_some());

    // Revoke capability
    process.capability_space.lock().revoke(cap_token);

    // Verify it's gone
    assert!(process.capability_space.lock().lookup(cap_token).is_none());
...
fn test_ipc_with_shared_memory() {
    ...
    use veridian_kernel::ipc::{Permissions, SharedRegion, TransferMode};
