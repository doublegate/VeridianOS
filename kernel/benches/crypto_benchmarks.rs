...
//!
//! Benchmarks for hash functions, encryption, signatures, and post-quantum
//! crypto.
...
use veridian_kernel::{
    bench_function,
    crypto::{
        asymmetric::{key_exchange::SecretKey, SigningKey},
        hash::{blake3, sha256, sha512},
        post_quantum::{DilithiumLevel, DilithiumSigningKey, KyberLevel, KyberSecretKey},
        symmetric::{Aes256Gcm, ChaCha20Poly1305, SymmetricCipher},
    },
...
fn bench_sha256_1kb() {
    ...
    let (duration_ns, _) = bench_function(|| sha256(&data));
...
fn bench_sha512_1kb() {
    ...
    let (duration_ns, _) = bench_function(|| sha512(&data));
...
fn bench_blake3_1kb() {
    ...
    let (duration_ns, _) = bench_function(|| blake3(&data));
...
fn bench_aes256gcm_encrypt_1kb() {
    ...
    let (duration_ns, _) =
        bench_function(|| cipher.encrypt(&data, &nonce).expect("Encryption failed"));
...
fn bench_ed25519_sign() {
    ...
    let (duration_ns, _) = bench_function(|| signing_key.sign(message).expect("Signing failed"));
...
fn bench_dilithium2_sign() {
    ...
    let signing_key = DilithiumSigningKey::generate(DilithiumLevel::Level2).expect("Keygen failed");
...
fn bench_kyber768_keygen() {
    ...
    let (duration_ns, _) =
        bench_function(|| KyberSecretKey::generate(KyberLevel::Kyber768).expect("Keygen failed"));
...
fn bench_kyber768_encapsulate() {
    ...
    let secret_key = KyberSecretKey::generate(KyberLevel::Kyber768).expect("Keygen failed");
    let public_key = secret_key.public_key();

    let (duration_ns, _) =
        bench_function(|| public_key.encapsulate().expect("Encapsulation failed"));
...
fn bench_aslr_address_randomization() {
    ...
    let (duration_ns, _) = bench_function(|| aslr.randomize_address(0x400000, RegionType::Stack));

    serial_println!(
        "{} ns ({:.2} M ops/sec)",
        duration_ns,
        1000.0 / duration_ns as f64
    );
}
...
fn bench_stack_canary_generation() {
    ...
    let (duration_ns, _) = bench_function(|| StackCanary::new());
