...
pub fn sys_write(fd: usize, buffer: usize, count: usize) -> SyscallResult {
    ...
    // Create buffer slice
    let buffer_slice = unsafe {
        core::slice::from_raw_parts(buffer as *const u8, count)
    };
...
/// - flags: Mount flags
pub fn sys_mount(_device: usize, mount_point: usize, fs_type: usize, flags: usize) -> SyscallResult {
    // Validate pointers
    ...
    // Mount filesystem
    match VFS.write().mount_by_type(mount_path, fs_type_str, flags as u32) {
        Ok(_) => Ok(0),
        Err(_) => Err(SyscallError::InvalidState),
    }
}
