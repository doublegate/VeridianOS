...
impl ThreadManager {
    ...
    pub fn create_thread(
        ...
        let handle = Arc::new(ThreadHandle::new(
            thread_id,
            process_id,
        ...
        if let Some(process) = get_process(process_id) {
            ...
            crate::println!(
                "[THREAD] Created thread {} in process {} with stack at 0x{:x}",
                thread_id.0,
                process_id.0,
                stack_addr
            );
    ...
    pub fn set_thread_priority(
        ...
        if let Some(process) = get_process(handle.process_id) {
            if let Some(_thread) = process.get_thread(thread_id) {
                ...
                crate::println!(
                    "[THREAD] Updated thread {} priority (tracked in handle)",
                    thread_id.0
                );
            }
        }
    ...
    pub fn set_cpu_affinity(&self, thread_id: ThreadId, cpu_mask: u64) -> Result<(), KernelError> {
        ...
        if let Some(process) = get_process(handle.process_id) {
            if let Some(thread) = process.get_thread(thread_id) {
                ...
                crate::println!(
                    "[THREAD] Set thread {} CPU affinity to 0x{:x}",
                    thread_id.0,
                    cpu_mask
                );
                return Ok(());
    ...
    pub fn get_tls_value(&self, thread_id: ThreadId, key: TlsKey) -> Result<*mut u8, KernelError> {
        ...
        if let Some(process) = get_process(handle.process_id) {
            if let Some(thread) = process.get_thread(thread_id) {
                #[cfg(feature = "alloc")]
                {
                    let value = thread.get_tls_value(key.0 as u64).unwrap_or(0) as *mut u8;
                    return Ok(value);
                }
