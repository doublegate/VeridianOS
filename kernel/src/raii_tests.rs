...
use crate::{
    mm::{FRAME_ALLOCATOR, FrameAllocatorError},
    ...
    // Allocate a frame with RAII
    {
        match FRAME_ALLOCATOR.allocate_frame_raii() {
            Ok(frame_guard) => {
                ...
                // Frame will be automatically freed when frame_guard goes out of scope
            }
            Err(e) => {
...
fn test_frame_guard_leak() {
    ...
    println!("Frame leaked at {:#x} - must be manually freed", leaked_frame.addr());
...
fn test_capability_guard() {
    use crate::cap::{CapabilitySpace, Rights, ObjectRef};
    use alloc::sync::Arc;
    use spin::Mutex;
