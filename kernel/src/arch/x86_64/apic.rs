...
fn rdmsr(msr: u32) -> u64 {
    ...
    // well-known MSR address (IA32_APIC_BASE). This is a privileged,
    // read-only operation with no side effects beyond returning the MSR value.
    unsafe {
        core::arch::asm!(
            "rdmsr",
            in("ecx") msr,
...
fn phys_to_virt(phys: usize) -> Option<usize> {
