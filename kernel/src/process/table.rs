...
impl ProcessTable {
    ...
    pub fn add_process(&self, process: Process) -> Result<ProcessId, &'static str> {
        ...
        entries.insert(pid, ProcessEntry {
            process: Box::new(process),
            ref_count: 1,
        });
        
        self.process_count.fetch_add(1, core::sync::atomic::Ordering::Relaxed);
        Ok(pid)
    }
    ...
    pub fn remove_process(&self, pid: ProcessId) -> Option<Box<Process>> {
        ...
        if let Some(entry) = entries.remove(&pid) {
            self.process_count.fetch_sub(1, core::sync::atomic::Ordering::Relaxed);
            Some(entry.process)
        } else {
            None
        }
    ...
    pub fn count(&self) -> usize {
        self.process_count.load(core::sync::atomic::Ordering::Relaxed)
    }
